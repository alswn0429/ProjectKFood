...
public class BoardDAO4 {
	...
	public void insertBoard(BoardBean4 bean) {
		...
		try {
			...
			String refsql = "select max(ref) from board4";	//ref의 값중 가장 큰값을 리턴해주세요
			...
			String sql = "insert into board4 values(board4_seq.NEXTVAL,?,?,?,?,sysdate,?,?,?,0,?)";	//board_seq.NEXTVAL(시퀀스)시퀀스에 들어가있는 값의 다음값을 넣어준다
	...
	public Vector<BoardBean4> getAllBoard(int start,int end) {
		
		//리턴할 객체 선언
		Vector<BoardBean4> v = new Vector<>();
		
		getCon();
		
		try {
			//쿼리준비
			String sql = "select * from (select A.*,Rownum Rnum from (select * from board4 order by ref desc , re_step asc)A)"
					+"where Rnum >= ? and Rnum <= ?";
			//쿼리 실행할객체 선언
			pstmt = con.prepareStatement(sql);
	...
	public BoardBean4 getOneBoard(int num) {
		BoardBean4 bean = new BoardBean4();
		getCon();
		
		try {
			//조회수 증가 쿼리
			String readsql = "update board4 set readcount = readcount+1 where num=?";
	...
	//답변글이 저장 되는 메소드
	public void reWriteBoard(BoardBean4 bean) {
		...
		try {
			...
			String levelsql = "update board4 set re_level = re_level+1 where ref=? and re_level > ?";
			...
			//답변글 데이터를 저장
			String sql = "insert into board4 values(board4_seq.NEXTVAL,?,?,?,?,sysdate,?,?,?,0,?)";
			pstmt=con.prepareStatement(sql);
	...
	public BoardBean4 getOneUpdateBoard(int num) {
		BoardBean4 bean = new BoardBean4();
		getCon();
	...
	public String getPass(int num) {
		...
		try {
			String sql = "select password from board4 where num=?";
	...
	public void updateBoard(BoardBean4 bean) {
		getCon();
		
		try {
			String sql = "update board4 set subject=? , content=? where num=?";
